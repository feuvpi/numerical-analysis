...
main :: IO ()
main = do
    let f x = x**2 - 4 -- Example function: f(x) = xˆ2 -4
    let result = bisection f 0 3 0.0001 -- Apply bisection on the interval [0,3]
    case result of
        Just root -> putStrLn $ "Root found: " ++ show root
        Nothing -> putStrLn "No root in the given interval" 